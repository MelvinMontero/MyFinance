...
### Changed
- **Días de pago por defecto: 1 y 15** (antes 15 y 30). Con el modelo de quincena Q1 = 1–14 / Q2 = 15–fin, los pagos del 15 y 30 caían **los dos** en Q2 y Q1 quedaba sin ingreso. Con 1 y 15 cada pago aterriza en su propia quincena. Sugerencia en el formulario: si te pagan a fin de mes, anotá el día **1** (ese pago cuenta para la quincena nueva, no para el mes que ya pasó).
- **Rebajos (gastos fijos) repartidos 50/50 entre las dos quincenas** del mes, en vez de amortizarlos según los días que falten hasta el cobro. Cada fijo mensual aporta la mitad por quincena (moto 25k + 25k, etc.), tal como se lleva a mano. Si ya está pagado este mes, su provisión es 0.
...
### Fixed
- **Confirmar el pago de una quincena no se reflejaba en el balance.** Con pagos el 15 y 30, la quincena "en curso" (Q1 = días 1–14) no contenía ninguna ocurrencia, así que marcar el pago no movía el ingreso confirmado. Se corrige alineando los días de pago a 1 y 15 (cada pago cae en su quincena) — ver Changed.
- **Editar los días de pago ahora reproyecta las ocurrencias futuras.** Antes `updateIncome` no regeneraba nada, así que cambiar los días de pago no tenía efecto sobre las ocurrencias ya creadas. Ahora, cuando cambia el calendario (start_date, end_date, payday_1/2), se borran las ocurrencias futuras **sin confirmar** y se regeneran con el nuevo calendario; las pasadas y las ya confirmadas (dinero recibido) no se tocan.
